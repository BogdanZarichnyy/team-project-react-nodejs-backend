...
const addAdValidationSchema = Joi.object({
  name: Joi.string().pattern(regExp.nameRegExp).min(2).max(16).required(),
  family: Joi.string().pattern(regExp.stringRegExp).allow('').optional(),
  birthDate: Joi.string().required(),
  breed: Joi.string().pattern(regExp.stringRegExp).min(2).max(24).required(),
...
const updateAdValidationSchema = Joi.object({
  name: Joi.string().pattern(regExp.nameRegExp).min(2).max(16).optional(),
  family: Joi.string().pattern(regExp.stringRegExp).optional(),
  birthDate: Joi.string().optional(),
  breed: Joi.string().pattern(regExp.stringRegExp).min(2).max(24).optional(),
